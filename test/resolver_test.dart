...
void main() {
  group('the dictionary (M3-17)', () {
    ...
    test('an END inside an IF arm closes the section scope (msg 179)', () {
      final SemanticResult result = runJob(
        data: const [],
        procedure: [
          '      S1.         BEGIN SECTION.',
          "      DUP.        DISPLAY 'A'.",
          "            IF 1 = 1 THEN DISPLAY 'B' OTHERWISE END S1.",
          "      DUP.        DISPLAY 'C'.",
          '            STOP RUN.',
        ],
      );
      // The parser closes the section there and keeps compiling, so
      // the second DUP stands outside the section and collides with
      // nothing.
      expect(
        [
          for (final Diagnostic d in result.parse.parserDiagnostics)
            d.message.number,
        ],
        ['179,00'],
      );
      ...
      expect(
        [
          for (final DictionaryEntry e in result.dictionary.named('DUP'))
            e.section,
        ],
        ['S1', null],
      );
    ...
    test('the 3501st name draws 942,00 and stops; --no-table-limits '
      ...
      final SemanticResult capped = runJob(data: data);
      expect(ids(capped), ['942,00']);
  ...
  group('the triage (M3-17)', () {
    test('a reference declared nowhere draws 108,00', () {
      final SemanticResult result = runJob(
        data: [dataCard(name: 'T', level: '1', description: 'A(2)')],
    ...
    test('an ambiguous reference draws 166,00; qualification resolves '
      ...
      final SemanticResult bare = runJob(
        data: data,
        procedure: ['            MOVE MONTH TO T.'],
      );
      expect(ids(bare), ['166,00']);
      final SemanticResult qualified = runJob(
        data: data,
        procedure: ['            MOVE OLDREC MONTH TO T.'],
      );
      expect(ids(qualified), isEmpty);
    ...
    test('qualification is REDEF-blind: the qualified name of H is '
      ...
      expect(ids(result).where((String id) => id != '104,00'), isEmpty);
  ...
  group('condition names (M3-17)', () {
    test('a condition reference resolves to its COND entry', () {
      ...
      expect(
        result.dictionary.named('WED').single.kind,
        NameKind.dataCondition,
      );
    });
    ...
    test('an environment COND name is a keys condition '
      ...
      expect(
        result.dictionary.named('SENSE').single.kind,
        NameKind.keysCondition,
      );
    });

    test('a non-condition in condition position draws 25,00', () {
      final SemanticResult result = runJob(
        data: [dataCard(name: 'PLAIN', level: '1', description: '99')],
    ...
    test('an internal-decimal COND constant spends no digit on its '
        'sign (J 02.05.07 d)', () {
      List<String> signed(String constant) => [
        dataCard(name: 'N', level: '1', mode: 'I', description: '9(5)'),
        dataCard(name: 'NEG', level: '2', type: 'COND', description: constant),
      ];
      expect(ids(runJob(data: signed("'-12345'"))), isEmpty);
      expect(ids(runJob(data: signed("'12345+'"))), isEmpty);
      expect(ids(runJob(data: signed("'-123456'"))), ['37,00']);
    });

    test('an internal-decimal COND constant is measured against the '
        'clamped capacity', () {
      final SemanticResult result = runJob(
        data: [
          dataCard(
      ...
      // 35,00 is the mapper's clamp of the format to the register
      // form's 21 digits; 22 digits then overrun what the field holds.
      expect(ids(result), ['35,00', '37,00']);
    });
  });
  ...
  group('stray description names (J 02.05.06 e)', () {
    test('an unresolvable stray name draws 185,00', () {
      final SemanticResult result = runJob(
        data: [dataCard(name: 'X', level: '1', description: 'NOSUCH')],
      );
      expect(ids(result), contains('185,00'));
    });
    ...
    test('a stray name equal only to an environment name draws '
      ...
      expect(ids(result), ['185,00']);
    });

    test('unclaimed description tokens draw 185,00', () {
      final SemanticResult result = runJob(
        data: [dataCard(name: 'X', level: '1', description: "A(2) 'AB' 'CD'")],
      );
      expect(ids(result), ['185,00']);
    });
  });
