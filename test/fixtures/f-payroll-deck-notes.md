...
| 2–82 | `*PROCEDURE` header + 80 procedure cards (serials 01002–04022) |
| 83–190 | `*DATA` header + 107 data description cards (serials 05002–10015) |
...
- The listing prints a serial number in columns 1 to 6 of its own print line.
  It runs 01001 to 10015. **Serial 07006 does not appear.** The listing skips
  it between `NAME` of BONDORDER and the `CHECK` record header.

## Layout facts established from the scans

- **Serial columns 1 to 6 are blank on every card.** The listing prints its
  serial in a print column that no card column feeds.
...
- Procedure text starts in one of five columns, measured per card:
