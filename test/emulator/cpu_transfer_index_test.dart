...
void main() {
  ...
  group('TSX', () {
    ...
    test('the calling-sequence return convention works', () {
      ...
      m.write(0x100, typeB(0x03C, address: 0x300, tag: 4));
      m.write(0x300, typeB(0x010, address: 3, tag: 4)); // TRA 3,4
      m.ic = 0x100;
      cpu.step();
      cpu.step();
      expect(m.ic, 0x103);
  ...
  group('index transmission', () {
    ...
    test('a multiple-tag read is the OR of the named registers', () {
      ...
      m.xrWrite(1, 0x304); // 0o1404
      m.xrWrite(4, 0x631); // 0o3061
    ...
    test('LAC loads the 2\'s complement; CAL DATANAME,T reaches the base', () {
      ...
      m.write(0x100, typeB(0x15D, address: 0x200, tag: 4)); // LAC
      m.write(0x101, typeB(0x940, address: 5, tag: 4)); // CAL 5,4
      m.ic = 0x100;
      cpu.step();
    ...
    test('PXA clears the AC and places the index in the address part', () {
      m.acSign = 1;
      m.acMagnitude = MachineState.acQBit | 0x123;
      m.xrWrite(1, 0x77);
  ...
  group('CAS', () {
    ...
    test('minus magnitudes compare in reverse', () {
      m.acSign = 1;
      m.acMagnitude = 3;
      m.write(0x200, data(5, negative: true));
      cas(0x200); // -3 > -5.
      expect(m.ic, 0x101);
    });
    ...
    test('a 1 in P makes the AC magnitude larger than any word', () {
      m.acMagnitude = MachineState.acPBit;
      m.write(0x200, data(Word36.magnitudeMask));
      cas(0x200);
      expect(m.ic, 0x101);
    });
  });
  ...
  group('LAS', () {
    ...
    test('treats the S bit of Y as a magnitude bit', () {
      m.acMagnitude = MachineState.acPBit; // 2^35.
      m.write(0x200, data(0, negative: true)); // Also 2^35 unsigned.
      las(0x200);
