...
void main() {
  ...
  group('ANA, ANS, ORS', () {
    test('ANA masks into the AC and clears S and Q', () {
      ...
      m.write(0x200, data(0, negative: true)); // Only the S bit.
    ...
    test('ANS stores the AND into Y and keeps the AC', () {
      m.acMagnitude = MachineState.acPBit | 0xF0;
      m.write(0x200, data(0xFF, negative: true));
  ...
  test('the CAL-ANA-ORS masking idiom works end to end', () {
    m.write(0x200, data(0x3F << 6, negative: true)); // Flag source.
    m.write(0x201, data(0x3F << 6)); // Mask.
    m.write(0x202, 0); // Target.
    m.write(0x100, typeB(0x940, address: 0x200)); // CAL
    m.write(0x101, typeB(0x8D0, address: 0x201)); // ANA
    m.write(0x102, typeB(0x982, address: 0x202)); // ORS
    m.ic = 0x100;
    cpu.step();
    cpu.step();
    cpu.step();
