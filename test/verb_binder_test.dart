...
void main() {
  group('the verb sites (M3-18)', () {
    ...
    test('OPEN and CLOSE of a name that is no file draw 21,00', () {
      ...
      expect(_ids(result), ['21,00', '21,00']);
    });
    ...
    test('a file no verb processes draws 198,00', () {
      final SemanticResult result = _bind(
        [..._record('R1'), ..._record('R2')],
        environment: [
          ..._fileCards('FIN', ['INPUT', 'BCD', 'TAPE', 'R1', 'BLOCKSIZE 5']),
          ..._fileCards('FOUT', ['OUTPUT', 'BCD', 'TAPE', 'R2', 'BLOCKSIZE 5']),
        ],
        procedure: ['            GET R1.'],
      );
      expect(_ids(result), ['198,00']);
  ...
  group('GET RECORD FROM (M3-18; J 02.07.04)', () {
    ...
    List<String> inputFile(List<String> recordOptions) =>
        _fileCards('FIN', ['INPUT', 'BCD', 'TAPE', ...recordOptions]);
    ...
    test('a GET RECORD FROM an output file draws 14,00', () {
      ...
      expect(_ids(result), ['14,00']);
    });

    test('records of unequal fixed length draw 12,00', () {
      final SemanticResult result = _bind(
        [..._record('R1'), ..._record('R2', description: 'A(12)')],
        environment: inputFile(['R1', 'R2', 'BLOCKSIZE 5']),
        procedure: ['            GET RECORD FROM FIN.'],
      );
      expect(_ids(result), ['12,00']);
    });
    ...
    test('BEGIN rescues unequal lengths (J 02.07.04 b)', () {
      final SemanticResult result = _bind(
        [..._record('R1'), ..._record('R2', description: 'A(12)')],
        environment: inputFile(['R1', 'R2', 'BEGIN', 'BLOCKSIZE 5']),
        procedure: ['            GET RECORD FROM FIN.'],
      );
      expect(_ids(result), isEmpty);
    });
    ...
    test('PLACE LENGTH IN on one record of two draws 118,00', () {
      ...
      expect(_ids(result), ['118,00']);
    });

    test('BLOCK CONTROL on one record of two draws 121,00', () {
      final SemanticResult result = _bind(
        data(),
        environment: inputFile(['R1', 'BLOCK CONTROL', 'R2', 'BLOCKSIZE 5']),
        procedure: ['            GET RECORD FROM FIN.'],
      );
      expect(_ids(result), ['121,00']);
    });
  ...
  group('base locators (D9.7)', () {
    ...
    List<String> file(int records) => _fileCards('FIN', [
      'INPUT',
      'BCD',
      'TAPE',
      for (var i = 0; i < records; i++) 'R$i',
      'BLOCKSIZE 5',
    ]);

    test('the 128th located record draws 202,00 and stops; '
        '--no-table-limits lifts it', () {
      final SemanticResult capped = _bind(
        data(128),
        environment: file(128),
        procedure: ['            GET R0.'],
      );
      expect(_ids(capped), ['202,00']);
      expect(capped.stopped, isTrue);
      final SemanticResult lifted = _bind(
        data(128),
        environment: file(128),
        procedure: ['            GET R0.'],
        tableLimits: false,
