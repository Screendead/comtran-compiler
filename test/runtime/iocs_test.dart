...
void main() {
  group('IOC)8, the READ subroutine (J 90.02.08)', () {
    test('the records of a block locate in turn, then the next block', () {
      ...
      expect(subject.state.read(_buffer), 1);
      expect(subject.state.read(_buffer + 1), 2);
      ...
      expect(subject.run(maxSteps: 2).outcome, RunOutcome.stepLimit);
      expect(subject.state.read(_locator), pzeWord(address: _buffer + 2));
      // The third record spends the block, so it reads the next one and
      // locates at the base again.
      expect(subject.run(maxSteps: 2).outcome, RunOutcome.stepLimit);
      expect(subject.state.read(_locator), pzeWord(address: _buffer));
      expect(subject.state.read(_buffer), 5);
    ...
    test('a block enters the buffer BLOCKSIZE words deep', () {
      ...
      expect(subject.state.read(_locator), pzeWord(address: _buffer));
      expect(subject.state.read(_buffer + 2), 3);
      // The block's other eleven words are discarded, not buffered.
      expect(subject.state.read(_buffer + 3), 0);
    ...
    test('two files locate a record each, in their own buffers', () {
      ...
      expect(subject.state.read(_locator), pzeWord(address: _buffer));
      expect(subject.state.read(_locator2), pzeWord(address: _buffer + 2));
      expect(subject.state.read(_buffer), 1);
      expect(subject.state.read(_buffer + 2), 3);
    });
  });
  ...
  group('the AT END exit (D6.5; D6.6)', () {
    test('a file mark takes it, and the next GET reads on past it', () {
      ...
      expect(subject.state.read(_buffer), 3);
    });
  ...
  group('the terminators (J 90.02.28; J 90.02.32)', () {
    ...
    test('a frame that is no record takes SYS)283', () {
      final frames = <String, List<int>>{
        ...
        'the data is shorter than the length says': <int>[
          ...tapeField(12),
          ...tapeWord(1),
        ],
        'the length is no whole number of words': <int>[
          ...tapeField(4),
          1,
          2,
          3,
        ...
        'the image ends with no file mark': <int>[],
      };
      for (final MapEntry<String, List<int>> frame in frames.entries) {
        final Directory tapes = tempDirectory('comtran-tapes');
        File('${tapes.path}/D1.tap').writeAsBytesSync(frame.value);
        final Machine subject = machine(
        ...
        expect(
          subject.run(maxSteps: 6).outcome,
          RunOutcome.errorExit,
          reason: frame.key,
        );
        expect(subject.printed, <String>[
          'GET ERROR ON FILE1, BLOCK 1',
        ], reason: frame.key);
