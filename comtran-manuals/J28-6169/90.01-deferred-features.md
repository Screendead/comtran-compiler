...
<!-- conversion notes: All 5 pages (PDF 133-137) transcribed directly against page images; no image-only fallbacks were needed. Header code for PDF page 137 is listed as null in jmap.json but the printed header clearly reads 90.01.05, continuing the sequence from pages 133-136 (90.01.01-90.01.04); used that value. Item k) of the Internal Table Size Limitations table reads "-CP)+NN" exactly as printed, confirmed by glyph measurement at 400 dpi and by manual review (2026-08-01): the leading glyph is a hyphen/dash, metrically identical to the "Appox-Max" hyphen. It reads naturally as a dash introducing the notation "CP)+NN" — the manual's own compiler-generated-name form ("SYM)NNN", defined at 90.02.03), which has no opening parenthesis; an earlier version of this note wrongly conjectured a printing artifact of "(CP)+NN", a form that does not occur in this manual. Also noted: the closing parenthesis ending table item h) ("...requires 3 positional indicators.)") is editorial — in the original the parenthesis opened at "(each unique combination" is never closed; the ")" is retained here for readability. Overpunched/overbar digits do not occur in this chunk. -->
