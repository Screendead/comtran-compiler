from PIL import Image
import numpy as np, sys
BASE='/Users/jacklusher/development/comtran-compiler/comtran-manuals/F28-8043/images/page-%d.png'
page=int(sys.argv[1]); P=float(sys.argv[2]); PITCH=float(sys.argv[3])
a=np.array(Image.open(BASE%page).convert('L'))
dark=(a<128).sum(axis=1); bands=[];inr=False
for y,v in enumerate(dark):
    if v>8 and not inr: s=y;inr=True
    elif v<=8 and inr: bands.append((s,y));inr=False
def groups(y0,y1,gap=7):
    band=a[y0:y1]; c=(band<128).sum(axis=0); out=[];inr=False
    for x,v in enumerate(c):
        if v>0 and not inr: s=x;inr=True
        elif v==0 and inr: out.append((s,x));inr=False
    if inr: out.append((s,len(c)))
    m=[]
    for s,e in out:
        if m and s-m[-1][1]<gap: m[-1]=(m[-1][0],e)
        else: m.append((s,e))
    return m
lines=[]
for (y0,y1) in bands:
    h=y1-y0
    n=max(1,round(h/PITCH))
    if n==1: lines.append((y0,y1))
    else:
        step=h/n
        for k in range(n): lines.append((int(y0+k*step), int(y0+(k+1)*step)+1))
print('lines',len(lines))
for i,(y0,y1) in enumerate(lines):
    g=groups(y0,y1)
    if not g: print(i,'-'); continue
    x0=g[0][0]
    print(i,[round((s-x0)/P)+1 for s,e in g])
