...
section 2.6). `lib/src/loader/control_cards.dart` holds the one rule.

The fields follow [J 90.08.01] and [J 90.08.02]. Six points the manuals leave
...
deck.name and file number is ignored ([J 03.02.05]). A deck.name with an
imbedded blank is refused: it "will prevent execution of the object
...
bit 2 waives it. A card of deck type `001`, `010` or `011` is refused.
...
follow [J 90.03.04]: `00000` ends the card; `00001` takes `PZE` as an
absolute origin, `MON` as a relative origin, and `PTW` as a
reservation, and refuses `PTH`; `01111` names the entry point (D2.1); a
standard word relocates its decrement and its address by their classes,
a constant as is, a relative field by the origin, and a system reference
through the caller's table. The `00010` system reference point is a SYS
...
read, and the cards it consumed. The machine assembly stage writes the
