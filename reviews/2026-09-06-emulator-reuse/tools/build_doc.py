...
something better than either option this record offered.</strong> Our emulator
stays and D0.3 is not amended. On the 1963 source, this record recommended
reading it now as period evidence. Jack rejected that. The source is
<strong>sealed</strong>: no work reads it for any answer until the
reconstruction closes at M6, and a new milestone M7 then diffs our
reconstruction against it.</p>
<p>His reasoning, which is now decision D0.9: an unchecked reconstruction from
manuals is a demonstration that no reader can measure. A reconstruction held
apart from a surviving implementation, then compared, is a measured result.
Each agreement is evidence the method works. Each difference is either a real
change between January 1962 and July 1963, or an error in our recovery. He
also corrected a premise of this record: his goal was to revive COMTRAN, not a
particular version of it, and the January 1962 target was incidental to the
sources he could find when the project began.</p>
<p><strong>Item 3 is unanswered</strong> and stays open. Whether a differential
test harness gets built, and where it would live, is still his call. Nothing
waits on it.</p>
<p>The chips below are left as they were put to him, so the record shows the
