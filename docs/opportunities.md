...
| The first run of the machine assembly | 2026-09-06 | R6 and R7 |
...
### R6 — Align the MOVE and ADD scales in codegen

Rule 1 of this file sends this to section 6. It is scheduled roadmap work, not
a research candidate. `docs/HANDOVER.md` holds it under "Codegen defects the
...
The same run exposed a third codegen defect, and rule 1 sends it here for the
same reason as R6. Three call sites punch a digit count where the runtime
handler reads characters. A source pictorial with an `S` filler therefore makes
the handler read past the end of the field. `docs/HANDOVER.md` holds the three
sites.
