...
- **M6-8. What the generator cannot recover from the applied deck, and
  the choice it leaves. Open: Jack's call.** The generator refuses the
  ...
  - **B. Apply the remaining rows of §9.8.** Stage the arithmetic in a
    ...
    make MASTER.FILE binary.

  The roadmap's words are B, "with the documented F/J divergences
  applied". Under B the table items are two whole words, so the RET =
  INS defect reproduces by the same word-granular rule, and the
  sample's report becomes a value-level oracle for the corpus (M6-4).
  Under A the 11-character items meet defect 7 instead. The review
  record puts the choice to Jack, with B recommended for this stage and
  A recorded as a later stage.
