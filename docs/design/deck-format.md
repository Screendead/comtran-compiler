...
1. **Canon** (`.ctd`) — a binary punch-level card-image file. Canon files
   are the authoritative program sources. The compiler reads canon only.
2. **Mirror** (`.ct`) — a text rendering of a canon file. Mirrors are
...
The 90.05 mirror (`test/fixtures/90.05-payroll.ct`) predates this document and is
...
- Each canon file `X.ctd` has a committed mirror `X.ct`, generated by
...
## 7. The 2026-08-05 extension rename

Jack renamed the file extensions on 2026-08-05: a canon file now ends
`.ctd` (before: `.ctdeck`), and a mirror now ends `.ct` (before: `.deck`).
This is a file-naming change only. Three boundaries hold:
...
The sections above use the new extensions. A document that predates the
rename can name the old ones; the pairing rule is unchanged.
