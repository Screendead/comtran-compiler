...
**Status:** the language definition is complete and verified. Of its 74 Open Questions, 8 are resolved and 10 narrowed; the remainder are implementation decisions (e.g. per-message severity values) or genuinely unrecoverable. M0 (the design commitments, `docs/design/decisions.md`) is done; the punch-level deck format is frozen with its converter built (`docs/design/deck-format.md`). **M1 is done:** the card reader, the division scanners, statement numbering, and the compilation listing — `dart run comtran:comtranc tests/90.05-payroll.ctdeck` compiles the manual's own payroll sample through the front end and prints its listing, numbered exactly as the 1962 compile numbered it. The parser (M2) is next.
